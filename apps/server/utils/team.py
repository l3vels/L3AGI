...
from typings.team import TeamOutput, TeamInput, TeamWithConfigsOutput, ConfigsOutput
...
def convert_model_to_response(team_model: TeamModel) -> TeamWithConfigsOutput:
    team_data = {}
    
    # Extract attributes from AgentModel using annotations of Agent
    for key in TeamWithConfigsOutput.__annotations__.keys():
        if hasattr(team_model, key):
            target_type = TeamWithConfigsOutput.__annotations__.get(key)
            team_data[key] = convert_value_to_type(value=getattr(team_model, key), target_type=target_type)
        if key == "team_agents":
            team_data[key] = convert_team_agents_to_team_agent_list(getattr(team_model, key))
            continue
    ...
    return TeamWithConfigsOutput(agent=TeamModel(**team_data), 
                                    configs= ConfigsOutput(**configs) if configs else None)


def convert_teams_to_team_list(teams: List[TeamModel]) -> List[TeamWithConfigsOutput]:
    return [convert_model_to_response(team_model) for team_model in teams]