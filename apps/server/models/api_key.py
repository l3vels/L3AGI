...
class ApiKeyModel(BaseModel):
    ...
    name = Column(String)
    token = Column(String) # Later add as Enum
    ...
    def update_api_key(cls, db, id, api_key, user, account):
        ...
        db_api_key = cls.update_model_from_input(api_key_model=old_api_key, api_key_input=api_key)
    ...
    @classmethod
    def update_model_from_input(cls, api_key_model: ApiKeyModel, api_key_input: ApiKeyInput):
    ...
    def delete_by_id(cls, db, api_key_id, account):
        db_api_key = db.session.query(ApiKeyModel).filter(ApiKeyModel.id == api_key_id, ApiKeyModel.account_id==account.id).first()
