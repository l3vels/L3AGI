...
from l3_types.user_accout_model_types import UserAccountInput, UserAccountResponse, QueryParams
...
class UserAccountModel(BaseModel):
    ...
    def update_user_account(cls, db, id, user_account, user):
        ...
        db_user_account = cls.update_model_from_input(user_account_model=old_user_account, user_account_input=user_account)
    ...
    @classmethod
    def update_model_from_input(cls, user_account_model: 'UserAccountModel', user_account_input: UserAccountInput):
    ...
    def get_user_accounts(cls, db, query: QueryParams, account):
        filter_conditions = [or_(or_(UserAccountModel.is_deleted == False, UserAccountModel.is_deleted is None), UserAccountModel.is_deleted is None)]
    ...
    def delete_by_id(cls, db, user_account_id):
        db_user_account = db.session.query(UserAccountModel).filter(UserAccountModel.id == user_account_id).first()
