...
class ChatMessage(BaseModel):
    ...
    sender_user_id = Column(UUID,  ForeignKey('user.id', ondelete='CASCADE'), nullable=True, index=True)
    sender_account_id = Column(UUID, ForeignKey('account.id', ondelete='CASCADE'), nullable=False, index=True)
    ...
    sender_user = relationship("AccountModel", foreign_keys=[sender_account_id], lazy='select')
    sender_account = relationship("AccountModel", foreign_keys=[sender_account_id], lazy='select')
    @classmethod
    def get_chat_message_by_id(cls, db, chat_message_id: UUID, account: AccountOutput):
        ...
        chat_message = (
            ...
            .filter(ChatMessage.id == chat_message_id, ChatMessage.sender_account_id == account.id)
            .first()
        )
