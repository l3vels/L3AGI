...
from sqlalchemy import Column, String, Boolean, UUID, func, or_, ForeignKey, Index, Numeric
...
class ScheduleModel(BaseModel):
    ...
    configs = relationship("ScheduleConfigModel", back_populates="schedule", lazy='select')
    ...
    @classmethod
    def update_schedule(cls, db, id, schedule, configs: List[ConfigInput], user, account):
        """
        Creates a new schedule with the provided configuration.
        ...
        db_schedule = cls.update_model_from_input(schedule_model=old_schedule, schedule_input=schedule)
    ...
    @classmethod
    def update_model_from_input(cls, schedule_model: ScheduleModel, schedule_input: ScheduleInput):
        for field in ScheduleInput.__annotations__.keys():
            setattr(schedule_model, field, getattr(schedule_input, field))
        return schedule_model  
    
    @classmethod
    def create_schedule_from_template(cls, db, template_id, user, account, check_is_template: True):
        """
        Creates a new schedule with the provided configuration.
        ...
        """
        template_schedule = cls.get_schedule_by_id(db=db, schedule_id=template_id, account=account)
        if check_is_template:
            if template_schedule is None or not (template_schedule.is_public or template_schedule.is_template):
                raise ScheduleNotFoundException("Schedule not found")

        new_schedule = ScheduleModel(name=template_schedule.name,
        ...
        ScheduleConfigModel.create_configs_from_template(db=db, 
    ...
    def get_schedules(cls, db, account):
        schedules = (
            ...
            .filter(ScheduleModel.account_id == account.id, or_(or_(ScheduleModel.is_deleted == False, ScheduleModel.is_deleted is None), ScheduleModel.is_deleted is None))
    ...
    def get_template_schedules(cls, db):
        schedules = (
            db.session.query(ScheduleModel) 
            .filter(or_(ScheduleModel.is_deleted == False, ScheduleModel.is_deleted.is_(None)),
    ...
    def get_schedule_by_id(cls, db, schedule_id, account):
        ...
        schedule = (
            ...
            .join(ScheduleConfigModel, ScheduleModel.id == ScheduleConfigModel.schedule_id)
            .join(UserModel, ScheduleModel.created_by == UserModel.id)           
            .filter(ScheduleModel.id == schedule_id, or_(or_(ScheduleModel.is_deleted == False, ScheduleModel.is_deleted is None), ScheduleModel.is_deleted is None))
            .options(joinedload(ScheduleModel.configs))  # if you have a relationship set up named "configs"
            .options(joinedload(ScheduleModel.creator))
            # .options(joinedload(ScheduleModel.configs))  # if you have a relationship set up named "configs"
            # .options(joinedload(UserModel.schedules))
    ...
    def get_by_parent_id(cls, db, parent_id, account):
        ...
        schedule = (
            ...
            .join(ScheduleConfigModel, ScheduleModel.id == ScheduleConfigModel.schedule_id)
            .join(UserModel, ScheduleModel.created_by == UserModel.id)           
            .filter(ScheduleModel.parent_id == parent_id, 
                    ScheduleModel.account_id == account.id, 
                    or_(or_(ScheduleModel.is_deleted == False, ScheduleModel.is_deleted is None),
    ...
    def delete_by_id(cls, db, schedule_id, account):
        db_schedule = db.session.query(ScheduleModel).filter(ScheduleModel.id == schedule_id, ScheduleModel.account_id==account.id).first()
