...
class ScheduleConfigModel(BaseModel):
    ...
    schedule_id = Column(UUID(as_uuid=True), ForeignKey('schedule.id', ondelete='CASCADE'), index=True)
    ...
    __table_args__ = (Index('ix_schedule_config_model_schedule_id_key', 'schedule_id', 'key'),)
    ...
    def create_or_update(cls, db, schedule, update_configs, user, account):  
        ...
        """
        db_configs= db.session.query(ScheduleConfigModel).filter(
    ...
    def create_configs_from_template(cls, db, configs, user, schedule_id):  
        ...
        for template_config in configs:
            new_config = ScheduleConfigModel(key=template_config.key,
