...
def create_api_key(api_key: ApiKeyInput, auth: UserAccount = Depends(authenticate)) -> ApiKeyOutput:
    """
    ...
    db_api_key = ApiKeyModel.create_api_key(db, api_key=api_key, user=auth.user, account=auth.account)
    return convert_model_to_response(ApiKeyModel.get_api_key_by_id(db, db_api_key.id, auth.account))

@router.put("/{id}", status_code=200, response_model=ApiKeyOutput)  # Changed status code to 200
def update_api_key(id: str, api_key: ApiKeyInput, auth: UserAccount = Depends(authenticate)) -> ApiKeyOutput:
    """
    ...
    """
    try:
        db_api_key = ApiKeyModel.update_api_key(db, 
                                           id=id, 
                                           api_key=api_key, 
                                           user=auth.user, 
...
def get_api_key_by_id(id: str, auth: UserAccount = Depends(authenticate)) -> ApiKeyOutput:
    """
    ...
    if not db_api_key or db_api_key.is_deleted:
        raise HTTPException(status_code=404, detail="ApiKey not found")  # Ensure consistent case in error messages
