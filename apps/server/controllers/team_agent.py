...
def get_team_agent_by_id(id: str, auth: UserAccount = Depends(authenticate)) -> TeamAgentResponse:
    ...
    if not db_team_agent or db_team_agent.is_deleted:
        raise HTTPException(status_code=404, detail="TeamAgent not found")  # Ensure consistent case in error messages
...
def delete_team_agent(team_agent_id: str, auth: UserAccount = Depends(authenticate)):
    ...
    """
    try:
        TeamAgentModel.delete_by_id(db, team_agent_id=team_agent_id, account=auth.account)
