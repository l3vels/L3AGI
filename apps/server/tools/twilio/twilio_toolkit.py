...
class TwilioSearchToolkit(BaseToolkit, ABC):
    ...
    def get_env_keys(self) -> List[ToolEnvKey]:
        return [
            ToolEnvKey(label="Account SID", key="TWILIO_ACCOUNT_SID", key_type=ToolEnvKeyType.STRING, is_required=True, is_secret=False),
            ToolEnvKey(label="Auth Token", key="TWILIO_AUTH_TOKEN", key_type=ToolEnvKeyType.STRING, is_required=True, is_secret=True),
            ToolEnvKey(label="From Number", key="TWILIO_FROM_NUMBER", key_type=ToolEnvKeyType.STRING, is_required=True, is_secret=False),
        ]
